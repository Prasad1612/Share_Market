...
NseKitAsync.py  v6.0
====================
Async drop-in replacement for NseKit v5.0.
Every public method preserved in the exact same order as the original.

Key changes vs sync version
----------------------------
• aiohttp replaces requests  → true async I/O, no GIL blocking
• asyncio.gather() for concurrent multi-fetch  (N× faster than sequential)
• All _fetch / _get / _post unified into one _request() core
• asyncio.Lock replaces threading.RLock throughout
• async def / await on every data method
• Backward-compat shims for _build_headers, _prepare_request, _period_to_dates

Usage
-----
    import asyncio
    from NseKitAsync import AsyncNse, AsyncNseSession, sync_fetch

    async def main():
        async with AsyncNse() as nse:
            df = await nse.index_live_all_indices_data()

            # concurrent  —  all fire at the same time
            mkt, turn, idx = await asyncio.gather(
...
from typing import Any, AsyncIterator, Callable, Final, Iterator, TypeVar, ParamSpec
...
__all__ = [
    "AsyncNse", "AsyncNseSession", "AsyncNseQueryBuilder",
    "Period", "CachePolicy", "RateLimitConfig", "CacheConfig",
    "nse_api", "sync_fetch",
]
...
class _AsyncRateLimiter:
    ...
    async def on_429(self, backoff: float | None = None) -> None:
        async with self._lock:
            self._total_429s += 1
            self._backoff = float(backoff) if backoff is not None else min(max(self._backoff * 2, 2.0), 60.0)

    async def reset_backoff(self) -> None:
        async with self._lock:
            self._backoff = 0.0

    def configure(self, **kwargs: Any) -> "_AsyncRateLimiter":
    ...
    def stats(self) -> dict:
        return {
            "total_calls": self._total_calls, "total_429s": self._total_429s,
...
class _AsyncCookieVault:
    ...
    def __new__(cls) -> "_AsyncCookieVault":
        if cls._instance is None:
            ...
            inst._lock: asyncio.Lock | None = None  # type: ignore[attr-defined]
...
# ═══════════════════════════════════════════════════════════════════════════════
# Date helpers (pure, module-level)
# ═══════════════════════════════════════════════════════════════════════════════
...
class AsyncNse:
    """
    Async NSE / SEBI data client.
    All public methods are async def — use with await.
    Method order mirrors the original NseKit.Nse class exactly.
    ...
    def __init__(
        ...
        if verbose:
            ...
            if not log.handlers:
                ...
                log.addHandler(h)
        rl_cfg = RateLimitConfig(max_per_second=max_per_second,
                                  max_per_minute=max_per_minute, min_gap=min_gap)
    ...
    # ── property wrapping private _rate_limiter ───────────────────────────────

    @property
    def rate_limiter(self) -> _AsyncRateLimiter:
        return self._rate_limiter

    # ── cached_property shorthand ─────────────────────────────────────────────
    ...
    def __repr__(self) -> str:
        ...
        return (f"<AsyncNse calls={rl['total_calls']} "
                f"429s={rl['total_429s']} cache={self._cache.stats()}>")
    ...
    def __enter__(self):
        raise RuntimeError("Use 'async with AsyncNse() as nse:' not sync 'with'")
    ...
    def _build_headers(self) -> dict:
        """Backward-compat shim — delegates to _base_headers()."""
    ...
    async def _warm_up(self) -> None:
        ...
        try:
            ...
            async with self._session.get(
                ...
                if r.ok:
                    await self._cookie_vault.inject({k: v.value for k, v in r.cookies.items()})
            await asyncio.sleep(_WARMUP_WAIT)
        except Exception:
            pass

    def rotate_user_agent(self) -> None:
        """Rotate UA every 5 calls (handled internally by _rotate_ua)."""
        pass
    ...
    async def _prepare_request(self, extra_headers=None, ref_url=None, timeout=10):
    ...
    def _validate_symbol(symbol: str) -> str:
        ...
        if not re.fullmatch(r"[A-Z0-9&\-]+", cleaned):
            raise ValueError(f"symbol {cleaned!r} contains invalid characters")
        return cleaned

    # ── _period_to_dates instance shim ────────────────────────────────────────

    def _period_to_dates(self, *args: Any, from_date=None, to_date=None,
                          period=None, default_period="1Y") -> tuple[str, str]:
        return _period_to_dates(*args, from_date=from_date, to_date=to_date,
                                period=period, default_period=default_period)
    ...
    async def _request(
        ...
        assert self._session, "Use 'async with AsyncNse() as nse:'"
        hdrs = self._rotate_ua()
        if extra_headers:
            hdrs = {**hdrs, **extra_headers}
        if ref_url and ref_url not in (_NSE_ROOT, "https://www.nseindia.com/"):
            try:
                async with self._session.get(ref_url, headers=hdrs,
        ...
        for attempt in range(1, retries + 1):
            ...
            self._call_count += 1
            try:
                kw: dict = dict(headers=hdrs, timeout=aiohttp.ClientTimeout(total=timeout))
                if params: kw["params"] = params
                if data:   kw["data"]   = data
                cm = self._session.post(url, **kw) if method.upper() == "POST" \
                     else self._session.get(url, **kw)
                async with cm as resp:
                    ...
                    if fresh: await self._cookie_vault.inject(fresh)
                    if not is_json:
                        return await resp.read()
                    try:
                        return await resp.json(content_type=None)
                    except Exception:
                        log.warning("[AsyncNse] non-JSON → %s", url[:80])
                        return None
    ...
    # ── _fetch / _get / _post thin wrappers ──────────────────────────────────
    ...
    async def _post(self, url: str, *, data: dict | None = None,
        ...
        return await self._request(url, method="POST", data=data,
                                   extra_headers=extra_headers, **kw)

    # ── chunked date-range downloader ─────────────────────────────────────────

    async def _chunk_fetch(self, ref_url: str, base_api_template: str,
                            from_date: str, to_date: str, *,
                            chunk_days: int = 89, max_retries: int = 3,
                            data_key: str = "data") -> list:
        results: list = []
        async for rec in self._chunk_fetch_iter(ref_url, base_api_template,
                                                 from_date, to_date,
    ...
    async def _chunk_fetch_iter(self, ref_url: str, base_api_template: str,
                                 from_date: str, to_date: str, *,
                                 chunk_days: int = 89, max_retries: int = 3,
                                 data_key: str = "data"):
        ...
        while start_dt <= end_dt:
            ...
            api_url   = base_api_template.format(
                start_dt.strftime("%d-%m-%Y"), chunk_end.strftime("%d-%m-%Y"))
            payload = await self._request(api_url, ref_url=ref_url,
    ...
    async def clear_cache(self) -> None:
        await self._cache.clear()
        log.debug("[AsyncNse] cache cleared")

    def cache_stats(self) -> dict:
        return self._cache.stats()

    def no_cache(self) -> _AsyncNoCacheCtx:
        """Async context manager — bypass cache for enclosed block."""
        return _AsyncNoCacheCtx()

    # ─────────────────────────────────────────────────────────────────────────
    # Private HTTP helpers  (_get / _post) — already defined above as wrappers
    # The section below matches the original file section order
    # ─────────────────────────────────────────────────────────────────────────
    ...
    async def nse_market_status(self, mode: str = "Market Status") -> pd.DataFrame | dict | None:
        data = await self._fetch("https://www.nseindia.com/market-data/live-equity-market",
        ...
        if ms is not None:
            keep = ["market","marketStatus","tradeDate","index","last","variation",
                    "percentChange","marketStatusMessage"]
            ms = ms[[c for c in keep if c in ms.columns]]
        mc  = _df(data.get("marketcap"), {
            "timeStamp":"Date","marketCapinTRDollars":"MarketCap_USD_Trillion",
            "marketCapinLACCRRupees":"MarketCap_INR_LakhCr","marketCapinCRRupees":"MarketCap_INR_Cr"})
        n50 = _df(data.get("indicativenifty50"), {
            "dateTime":"DateTime","indexName":"Index","closingValue":"ClosingValue",
            "finalClosingValue":"FinalClose","change":"Change","perChange":"PercentChange"})
        gn  = _df(data.get("giftnifty"), {
            "SYMBOL":"Symbol","EXPIRYDATE":"ExpiryDate","LASTPRICE":"LastPrice",
            "DAYCHANGE":"DayChange","PERCHANGE":"PercentChange",
            "CONTRACTSTRADED":"ContractsTraded","TIMESTMP":"Timestamp"})
    ...
    async def nse_is_market_open(self, market: str = "Capital Market") -> Text:
        data = await self._fetch("https://www.nseindia.com/market-data/live-equity-market",
                                  "https://www.nseindia.com/api/marketStatus")
        if not data: return Text("Error fetching NSE Market Status", style="bold red")
        sel = next((m for m in data.get("marketState", []) if m.get("market") == market), None)
        if not sel: return Text(f"[{market}] → Market data not found.", style="bold yellow")
        msg  = sel.get("marketStatusMessage", "").strip()
        text = Text(f"[{market}] → ", style="bold white")
        text.append(msg, style="bold red" if any(w in msg.lower()
                    for w in ("closed","halted","suspended")) else "bold green")
        return text
    ...
    async def nse_trading_holidays(self, list_only: bool = False) -> pd.DataFrame | list | None:
        ...
        df = pd.DataFrame(data["CM"], columns=["Sr_no","tradingDate","weekDay",
                                                 "description","morning_session","evening_session"])
    ...
    async def nse_clearing_holidays(self, list_only: bool = False) -> pd.DataFrame | list | None:
        ...
        df = pd.DataFrame(data["CD"], columns=["Sr_no","tradingDate","weekDay",
                                                 "description","morning_session","evening_session"])
        return df["tradingDate"].tolist() if list_only else df
    ...
    async def nse_live_market_turnover(self) -> pd.DataFrame:
        data = await self._fetch("https://www.nseindia.com/option-chain",
        ...
        df.replace([pd.NA, float("nan"), float("inf"), float("-inf")], None, inplace=True)
        return df

    @nse_api(ttl=15.0)
    async def nse_live_hist_circulars(self, from_date_str=None, to_date_str=None, filter=None) -> pd.DataFrame:
        today = datetime.now()
        ...
        data = await self._fetch("https://www.nseindia.com/resources/exchange-communication-circulars",
        ...
        df = pd.DataFrame(items).rename(columns={
            "cirDisplayDate":"Date","circDisplayNo":"Circulars No","circCategory":"Category",
            "circDepartment":"Department","sub":"Subject","circFilelink":"Attachment"})
        df = df[[c for c in _cols if c in df.columns]]
        if filter: df = df[df["Department"].str.contains(filter, case=False, na=False)]
        return df.reset_index(drop=True)

    @nse_api(ttl=15.0)
    async def nse_live_hist_press_releases(self, from_date_str=None, to_date_str=None, filter=None) -> pd.DataFrame:
        today = datetime.now()
        from_date_str = from_date_str or (today - timedelta(days=1)).strftime("%d-%m-%Y")
        to_date_str   = to_date_str   or today.strftime("%d-%m-%Y")
        empty = pd.DataFrame(columns=["DATE","DEPARTMENT","SUBJECT","ATTACHMENT URL","LAST UPDATED"])
        data  = await self._fetch("https://www.nseindia.com/resources/exchange-communication-press-releases",
    ...
    async def nse_reference_rates(self) -> pd.DataFrame | None:
        ...
        rates = (raw or {}).get("currencySpotRates") or (raw or {}).get("spotRates") or \
    ...
    async def nse_eom_fno_full_list(self, mode: str = "stocks", list_only: bool = False) -> pd.DataFrame | list | None:
        ...
        if not data: return None
        try: raw = data["data"]["IndexList"] if mode.strip().lower()=="index" else data["data"]["UnderlyingList"]
    ...
    async def list_of_indices(self, as_dataframe: bool = False) -> dict | pd.DataFrame | None:
        data = await self._get("https://www.nseindia.com/api/equity-master",
    ...
    async def state_wise_registered_investors(self) -> dict | None:
        return await self._fetch("https://www.nseindia.com/registered-investors/",
    ...
    async def ipo_current(self) -> pd.DataFrame | None:
        data = await self._fetch("https://www.nseindia.com/market-data/upcoming-issues-other-than-sme",
    ...
    async def ipo_preopen(self) -> pd.DataFrame | None:
        data = await self._fetch("https://www.nseindia.com/market-data/pre-open-market-cm-and-emerge-market",
                                  "https://www.nseindia.com/api/market-data-pre-open?key=ITPD")
        if not isinstance(data, dict) or not isinstance(data.get("data"), list): return None
        try:
            rows = [{"symbol":i["metadata"]["symbol"],"previousClose":i["metadata"]["previousClose"],
                     "iep":i["metadata"]["iep"],"change":i["metadata"]["change"],
                     "pChange":i["metadata"]["pChange"],"lastPrice":i["metadata"]["lastPrice"],
                     "finalQuantity":i["metadata"]["finalQuantity"],"totalTurnover":i["metadata"]["totalTurnover"],
    ...
    async def ipo_tracker_summary(self, filter: str | None = None) -> pd.DataFrame | None:
        data = await self._fetch("https://www.nseindia.com/ipo-tracker?type=ipo_year",
        ...
        if filter: df = df[df["MARKETTYPE"].str.contains(filter.upper(), case=False, na=False)]
        keep = ["SYMBOL","COMPANYNAME","LISTED_ON","ISSUE_PRICE","LISTED_DAY_CLOSE","LISTED_DAY_GAIN",
                "LISTED_DAY_GAIN_PER","LTP","GAIN_LOSS","GAIN_LOSS_PER","MARKETTYPE"]
    ...
    _PRE_OPEN_XREF = {"NIFTY 50":"NIFTY","Nifty Bank":"BANKNIFTY","Emerge":"SME",
    ...
    async def pre_market_nifty_info(self, category: str = "All") -> pd.DataFrame | None:
        ...
        return pd.DataFrame([{"lastPrice":ns.get("lastPrice","N/A"),"change":ns.get("change","N/A"),
                               "pChange":ns.get("pChange","N/A"),"advances":data.get("advances",0),
                               "declines":data.get("declines",0),"unchanged":data.get("unchanged",0),
    ...
    async def pre_market_all_nse_adv_dec_info(self, category: str = "All") -> pd.DataFrame | None:
        ...
        data = await self._fetch("https://www.nseindia.com/market-data/pre-open-market-cm-and-emerge-market",
                                  f"https://www.nseindia.com/api/market-data-pre-open?key={key}")
        if not data: return None
        return pd.DataFrame([{"advances":data.get("advances",0),"declines":data.get("declines",0),
                               "unchanged":data.get("unchanged",0),"timestamp":data.get("timestamp","Unknown")}])
    ...
    async def pre_market_info(self, category: str = "All") -> pd.DataFrame | None:
        ...
        try:
            rows = [{"symbol":i["metadata"]["symbol"],"previousClose":i["metadata"]["previousClose"],
                     ...
                     "pChange":i["metadata"]["pChange"],"lastPrice":i["metadata"]["lastPrice"],
                     "finalQuantity":i["metadata"]["finalQuantity"],"totalTurnover":i["metadata"]["totalTurnover"],
                     "marketCap":i["metadata"].get("marketCap"),"yearHigh":i["metadata"].get("yearHigh"),
                     "yearLow":i["metadata"].get("yearLow"),
                     "totalBuyQuantity":i["detail"]["preOpenMarket"]["totalBuyQuantity"],
                     "totalSellQuantity":i["detail"]["preOpenMarket"]["totalSellQuantity"],
            ...
            rows = [i.get("metadata",i) for i in data["data"]]
        df = pd.DataFrame(rows)
        if not df.empty and "symbol" in df.columns: df = df.set_index("symbol", drop=False)
    ...
    async def pre_market_derivatives_info(self, category: str = "Index Futures") -> pd.DataFrame | None:
        ...
        data = await self._fetch("https://www.nseindia.com/market-data/pre-open-market-fno",
                                  f"https://www.nseindia.com/api/market-data-pre-open-fno?key={key}")
        if not isinstance(data, dict) or not isinstance(data.get("data"), list): return None
        try:
            rows = [{"symbol":i["metadata"]["symbol"],"expiryDate":i["metadata"]["expiryDate"],
    ...
    async def index_live_all_indices_data(self) -> pd.DataFrame | None:
        data = await self._fetch("https://www.nseindia.com/market-data/index-performances",
    ...
    async def index_live_indices_stocks_data(self, category: str = "NIFTY 50", list_only: bool = False) -> pd.DataFrame | list | None:
        ...
        if df.empty: return None
        if "symbol" in df.columns: df = df.set_index("symbol", drop=False)
    ...
    async def index_live_contribution(self, *args: Any, Index: str = "NIFTY 50", Mode: str = "First Five") -> pd.DataFrame | None:
        ...
        if mode_ not in ("First Five","Full"): raise ValueError("Mode must be 'First Five' or 'Full'")
        ...
        return df[[c for c in cols if c in df.columns]].fillna(0).replace({float("inf"):0,float("-inf"):0}) if not df.empty else None
    ...
    @nse_api(ttl=300.0)
    async def index_historical_data(self, index: str, *args: Any,
                                      from_date=None, to_date=None, period=None) -> pd.DataFrame:
        from_date, to_date = _period_to_dates(*args, from_date=from_date, to_date=to_date, period=period)
        enc  = index.replace(" ","%20").upper()
        raw  = await self._chunk_fetch("https://www.nseindia.com/option-chain",
               f"https://www.nseindia.com/api/historicalOR/indicesHistory?indexType={enc}&from={{}}&to={{}}",
               from_date, to_date)
        if not raw: return pd.DataFrame()
        df = pd.DataFrame(raw).rename(columns={
            "EOD_TIMESTAMP":"Date","EOD_INDEX_NAME":"Index Name",
            "EOD_OPEN_INDEX_VAL":"Open","EOD_HIGH_INDEX_VAL":"High",
            "EOD_LOW_INDEX_VAL":"Low","EOD_CLOSE_INDEX_VAL":"Close",
            "HIT_TRADED_QTY":"Shares Traded","HIT_TURN_OVER":"Turnover (₹ Cr)"})
        df = df[[c for c in ["Date","Index Name","Open","High","Low","Close","Shares Traded","Turnover (₹ Cr)"] if c in df.columns]]
        df["Date"] = pd.to_datetime(df["Date"], format="%d-%b-%Y", errors="coerce")
        df.sort_values("Date", inplace=True); df.drop_duplicates(subset=["Date"], keep="last", inplace=True)
        df["Date"] = df["Date"].dt.strftime("%d-%b-%Y")
        return df.reset_index(drop=True)

    @nse_api(ttl=300.0)
    async def index_pe_pb_div_historical_data(self, index: str, *args: Any,
                                               from_date=None, to_date=None, period=None) -> pd.DataFrame:
        from_date, to_date = _period_to_dates(*args, from_date=from_date, to_date=to_date, period=period)
        enc = index.replace(" ","%20").upper()
        raw = await self._chunk_fetch("https://www.nseindia.com/option-chain",
              f"https://www.nseindia.com/api/historicalOR/indicesYield?indexType={enc}&from={{}}&to={{}}",
              from_date, to_date)
        if not raw: return pd.DataFrame()
        df = pd.DataFrame(raw)
        df = df[[c for c in ["IY_INDEX","IY_DT","IY_PE","IY_PB","IY_DY"] if c in df.columns]]
        df.rename(columns={"IY_INDEX":"Index Name","IY_DT":"Date","IY_PE":"P/E","IY_PB":"P/B","IY_DY":"Div Yield%"}, inplace=True)
        for col in ["P/E","P/B","Div Yield%"]:
            if col in df.columns: df[col] = pd.to_numeric(df[col], errors="coerce")
        df.replace([float("inf"),float("-inf")], None, inplace=True)
        df.ffill(inplace=True); df.dropna(subset=["P/E"], inplace=True)
        df["Date"] = pd.to_datetime(df["Date"], format="%d-%b-%Y", errors="coerce")
        df.sort_values("Date", inplace=True); df.drop_duplicates(subset=["Date"], keep="last", inplace=True)
        df["Date"] = df["Date"].dt.strftime("%d-%b-%Y")
        return df.reset_index(drop=True)
    ...
    @nse_api(ttl=300.0)
    async def india_vix_historical_data(self, *args: Any, from_date=None, to_date=None, period=None) -> pd.DataFrame:
        from_date, to_date = _period_to_dates(*args, from_date=from_date, to_date=to_date, period=period)
        raw = await self._chunk_fetch("https://www.nseindia.com/option-chain",
              "https://www.nseindia.com/api/historicalOR/vixhistory?from={}&to={}", from_date, to_date)
        if not raw: return pd.DataFrame()
        df = pd.DataFrame(raw)
        expected = ["EOD_TIMESTAMP","EOD_INDEX_NAME","EOD_OPEN_INDEX_VAL","EOD_HIGH_INDEX_VAL",
                    "EOD_LOW_INDEX_VAL","EOD_CLOSE_INDEX_VAL","EOD_PREV_CLOSE","VIX_PTS_CHG","VIX_PERC_CHG"]
        df = df[[c for c in expected if c in df.columns]]
        df.rename(columns={"EOD_TIMESTAMP":"Date","EOD_INDEX_NAME":"Symbol",
                            "EOD_OPEN_INDEX_VAL":"Open Price","EOD_HIGH_INDEX_VAL":"High Price",
                            "EOD_LOW_INDEX_VAL":"Low Price","EOD_CLOSE_INDEX_VAL":"Close Price",
                            "EOD_PREV_CLOSE":"Prev Close","VIX_PTS_CHG":"VIX Pts Chg",
                            "VIX_PERC_CHG":"VIX % Chg"}, inplace=True)
        df["Date"] = pd.to_datetime(df["Date"], format="%d-%b-%Y", errors="coerce")
        df.sort_values("Date", inplace=True); df.drop_duplicates(subset=["Date"], keep="last", inplace=True)
        df["Date"] = df["Date"].dt.strftime("%d-%b-%Y")
        return df.reset_index(drop=True)
    ...
    async def cm_live_gifty_nifty(self) -> pd.DataFrame | None:
        ...
        if not isinstance(data, dict) or "data" not in data: return None
        gift = data["data"].get("giftNifty",{}); usd = data["data"].get("usdInr",{})
        return pd.DataFrame([{"symbol":gift.get("symbol"),"lastprice":gift.get("lastprice"),
                               "daychange":gift.get("daychange"),"perchange":gift.get("perchange"),
                               "contractstraded":gift.get("contractstraded"),"timestmp":gift.get("timestmp"),
                               "expirydate":gift.get("expirydate"),"usdInr_symbol":usd.get("symbol"),
                               "usdInr_ltp":usd.get("ltp"),"usdInr_updated_time":usd.get("updated_time"),
                               "usdInr_expiry_dt":usd.get("expiry_dt")}])
    ...
    async def cm_live_market_statistics(self) -> pd.DataFrame | None:
        ...
        if isinstance(data, dict) and "data" not in data: data = {"data": data}
        d        = (data or {}).get("data",{})
        snapshot = d.get("snapshotCapitalMarket") or d.get("snapshot") or {}
        fw52     = d.get("fiftyTwoWeek") or d.get("52week") or {}
        circuit  = d.get("circuit") or {}
        reg_str  = str(d.get("regInvestors") or d.get("registeredInvestors") or "0")
        try: reg_cr = round(int(reg_str.replace(",","").strip())/1e7, 2)
        except: reg_cr = 0.0
        return pd.DataFrame([{"Total":snapshot.get("total"),"Advances":snapshot.get("advances"),
                               "Declines":snapshot.get("declines"),"Unchanged":snapshot.get("unchange"),
                               "52W High":fw52.get("high"),"52W Low":fw52.get("low"),
                               "Upper Circuit":circuit.get("upper"),"Lower Circuit":circuit.get("lower"),
                               "Market Cap ₹ Lac Crs":round(d.get("tlMKtCapLacCr",0),2),
                               "Market Cap Tn $":round(d.get("tlMKtCapTri",0),3),
                               "Registered Investors":reg_str,"Registered Investors (Cr)":reg_cr,
                               "Date":d.get("asOnDate")}])

    @nse_api(ttl=15.0)
    async def cm_live_equity_info(self, symbol: str) -> dict | None:
        enc  = self._validate_symbol(symbol).replace("&","%26")
        data = await self._fetch(f"https://www.nseindia.com/get-quotes/equity?symbol={enc}",
                                  f"https://www.nseindia.com/api/quote-equity?symbol={enc}")
        if not data: return None
        try:
            pi = data["priceInfo"]
            return {"Symbol":symbol,"PreviousClose":pi["previousClose"],"LastTradedPrice":pi["lastPrice"],
                    "Change":pi["change"],"PercentChange":pi["pChange"],"Open":pi["open"],
                    "Close":pi.get("close"),"High":pi["intraDayHighLow"]["max"],
                    "Low":pi["intraDayHighLow"]["min"],"VWAP":pi.get("vwap"),
    ...
    async def cm_live_equity_price_info(self, symbol: str) -> dict | None:
        enc  = self._validate_symbol(symbol).replace("&","%26")
        data = await self._fetch(f"https://www.nseindia.com/get-quotes/equity?symbol={enc}",
                                  f"https://www.nseindia.com/api/quote-equity?symbol={enc}&section=trade_info")
    ...
    async def cm_live_most_active_equity_by_value(self) -> pd.DataFrame | None:
        data = await self._fetch("https://www.nseindia.com/market-data/most-active-equities-by-value",
    ...
    async def cm_live_most_active_equity_by_vol(self) -> pd.DataFrame | None:
        data = await self._fetch("https://www.nseindia.com/market-data/most-active-equities-by-volume",
    ...
    async def cm_live_volume_spurts(self) -> pd.DataFrame | None:
        data = await self._fetch("https://www.nseindia.com/market-data/volume-gainers-spurts",
    ...
    async def cm_live_52week_high(self) -> pd.DataFrame | None:
        data = await self._fetch("https://www.nseindia.com/market-data/52-week-high-equity-market",
    ...
    async def cm_live_52week_low(self) -> pd.DataFrame | None:
        data = await self._fetch("https://www.nseindia.com/market-data/52-week-low-equity-market",
    ...
    async def cm_live_block_deal(self) -> pd.DataFrame | None:
        data = await self._fetch("https://www.nseindia.com/market-data/block-deal-watch",
    ...
    @nse_api(ttl=15.0)
    async def cm_live_hist_insider_trading(self, *args: Any, from_date=None, to_date=None,
                                            period=None, symbol=None) -> pd.DataFrame | None:
        today_str = datetime.now().strftime("%d-%m-%Y")
        for arg in args:
            if not isinstance(arg, str): continue
            if _DATE_RE.match(arg):         from_date = from_date or arg; to_date = to_date or (arg if from_date != arg else None)
        ...
        if symbol and from_date:   url = f"https://www.nseindia.com/api/corporates-pit?index=equities&from_date={from_date}&to_date={to_date}&symbol={symbol}"
        elif symbol:               url = f"https://www.nseindia.com/api/corporates-pit?index=equities&symbol={symbol}"
        else:                      url = f"https://www.nseindia.com/api/corporates-pit?index=equities&from_date={from_date}&to_date={to_date}"
        data    = await self._fetch("https://www.nseindia.com/companies-listing/corporate-filings-insider-trading", url)
        records = data.get("data") if isinstance(data, dict) else data
        if not records or not isinstance(records, list): return None
        df   = pd.DataFrame(records)
        cols = ["symbol","company","acqName","personCategory","secType","befAcqSharesNo","befAcqSharesPer",
                "remarks","secAcq","secVal","tdpTransactionType","securitiesTypePost","afterAcqSharesNo",
                "afterAcqSharesPer","acqfromDt","acqtoDt","intimDt","acqMode","derivativeType",
                "tdpDerivativeContractType","buyValue","buyQuantity","sellValue","sellquantity","exchange","date","xbrl"]
        return df[[c for c in cols if c in df.columns]].fillna("")

    @nse_api(ttl=15.0)
    async def cm_live_hist_corporate_announcement(self, *args: Any, from_date=None, to_date=None,
                                                   period=None, symbol=None) -> pd.DataFrame | None:
        today_str = datetime.now().strftime("%d-%m-%Y")
        for arg in args:
            if not isinstance(arg, str): continue
        ...
        if period:
            ...
            to_date   = to_date or today_str
        from_date = from_date or today_str; to_date = to_date or today_str
        if symbol and from_date:   url = f"https://www.nseindia.com/api/corporate-announcements?index=equities&from_date={from_date}&to_date={to_date}&symbol={symbol}&reqXbrl=false"
        elif symbol:               url = f"https://www.nseindia.com/api/corporate-announcements?index=equities&symbol={symbol}&reqXbrl=false"
        else:                      url = f"https://www.nseindia.com/api/corporate-announcements?index=equities&from_date={from_date}&to_date={to_date}&reqXbrl=false"
    ...
    @nse_api(ttl=15.0)
    async def cm_live_hist_corporate_action(self, *args: Any, from_date=None, to_date=None,
                                             period=None, symbol=None, filter=None) -> pd.DataFrame | None:
        ...
        if symbol and not any([from_date, to_date, period]):
            ...
            if period:
                from_date = (today-_DELTA_MAP.get(period,timedelta(days=365))).strftime("%d-%m-%Y")
                to_date   = to_date or today_str
        ...
        data    = await self._fetch("https://www.nseindia.com/companies-listing/corporate-filings-actions", url)
        records = data.get("data") if isinstance(data, dict) else data
        if not records or not isinstance(records, list): return None
        df = pd.DataFrame(records)
        df.rename(columns={"symbol":"SYMBOL","comp":"COMPANY NAME","series":"SERIES","subject":"PURPOSE",
                            "faceVal":"FACE VALUE","exDate":"EX-DATE","recDate":"RECORD DATE",
                            "bcStartDate":"BOOK CLOSURE START DATE","bcEndDate":"BOOK CLOSURE END DATE"}, inplace=True)
        if filter and "PURPOSE" in df.columns:
            df = df[df["PURPOSE"].str.contains(filter, case=False, na=False)]
        col_order = ["SYMBOL","COMPANY NAME","SERIES","PURPOSE","FACE VALUE","EX-DATE",
                     "RECORD DATE","BOOK CLOSURE START DATE","BOOK CLOSURE END DATE"]
        df = df[[c for c in col_order if c in df.columns]]
        return df.fillna("").replace({float("inf"):"",float("-inf"):""}) if not df.empty else None

    @nse_api(ttl=15.0)
    async def cm_live_today_event_calendar(self, from_date=None, to_date=None) -> pd.DataFrame | None:
        today_str = datetime.now().strftime("%d-%m-%Y")
        from_date = from_date or today_str; to_date = to_date or today_str
        data = await self._fetch("https://www.nseindia.com/companies-listing/corporate-filings-event-calendar",
                                  f"https://www.nseindia.com/api/event-calendar?index=equities&from_date={from_date}&to_date={to_date}")
        if not isinstance(data, list) or not data: return None
        df = pd.DataFrame(data)
        return df[[c for c in ["symbol","company","purpose","bm_desc","date"] if c in df.columns]] if not df.empty else None
    ...
    @nse_api(ttl=15.0)
    async def cm_live_hist_board_meetings(self, *args: Any, from_date=None, to_date=None,
                                           period=None, symbol=None) -> pd.DataFrame | None:
        for arg in args:
            if not isinstance(arg, str): continue
            if _DATE_RE.match(arg):         from_date = from_date or arg
        ...
        if period or from_date:
            from_date, to_date = _period_to_dates(from_date=from_date, to_date=to_date,
                                                   period=period, default_period="1D")
            url = (f"https://www.nseindia.com/api/corporate-board-meetings?index=equities"
                   f"&from_date={from_date}&to_date={to_date}"
                   + (f"&symbol={symbol}" if symbol else ""))
        elif symbol: url = f"https://www.nseindia.com/api/corporate-board-meetings?index=equities&symbol={symbol}"
        else:        url = "https://www.nseindia.com/api/corporate-board-meetings?index=equities"
        data    = await self._fetch("https://www.nseindia.com/companies-listing/corporate-filings-board-meetings", url)
    ...
    @nse_api(ttl=15.0)
    async def cm_live_hist_Shareholder_meetings(self, *args: Any, from_date=None, to_date=None,
                                                 period=None, symbol=None) -> pd.DataFrame | None:
        for arg in args:
            if not isinstance(arg, str): continue
            if _DATE_RE.match(arg):         from_date = from_date or arg
        ...
        data    = await self._fetch("https://www.nseindia.com/companies-listing/corporate-filings-postal-ballot",
    ...
    async def _corporate_further_issue(self, issue_type: str, *args: Any,
                                        from_date=None, to_date=None, period=None,
                                        symbol=None, stage=None) -> pd.DataFrame | None:
        today = datetime.now(); today_str = today.strftime("%d-%m-%Y")
        ...
        api_map = {"qip":"corporate-further-issues-qip","pref":"corporate-further-issues-pref",
                   "ri":"corporate-further-issues-ri"}
    ...
    async def cm_live_voting_results(self) -> pd.DataFrame | None:
        data = await self._fetch("https://www.nseindia.com/companies-listing/corporate-filings-voting-results",
    ...
    async def cm_live_qtly_shareholding_patterns(self) -> pd.DataFrame | None:
        data = await self._fetch("https://www.nseindia.com/companies-listing/corporate-filings-shareholding-pattern",
    ...
    async def cm_live_hist_br_sr(self, symbol: str) -> pd.DataFrame | None:
        data = await self._fetch("https://www.nseindia.com/companies-listing/corporate-filings-bsr",
    ...
    async def index_chart(self, index: str, period: str = "1M") -> dict | None:
        return await self._fetch("https://www.nseindia.com/market-data/live-equity-market",
    ...
    async def stock_chart(self, symbol: str, period: str = "1M") -> dict | None:
        ...
        return await self._fetch(f"https://www.nseindia.com/get-quotes/equity?symbol={sym}",
    ...
    async def fno_chart(self, symbol: str, period: str = "1M") -> dict | None:
        sym = self._validate_symbol(symbol)
        return await self._fetch("https://www.nseindia.com/option-chain",
                                  f"https://www.nseindia.com/api/chart-databyindex?index={sym}&indices=false&type={period.upper()}")
    ...
    @nse_api(ttl=15.0)
    async def symbol_specific_most_active_Calls_or_Puts_or_Contracts_by_OI(self, symbol: str, opt_type: str = "CE") -> pd.DataFrame | None:
        data = await self._fetch("https://www.nseindia.com/market-data/most-active-contracts",
    ...
    async def fno_live_futures_data(self, symbol: str = "NIFTY") -> pd.DataFrame | None:
        enc  = self._validate_symbol(symbol).replace("&","%26")
        data = await self._fetch(f"https://www.nseindia.com/get-quotes/derivatives?symbol={enc}",
                                  f"https://www.nseindia.com/api/NextApi/apiClient/GetQuoteApi"
        ...
        for col in ["openPrice","highPrice","lowPrice","closePrice","prevClose","lastPrice","change",
    ...
    async def fno_live_change_in_oi(self) -> pd.DataFrame | None:
        data = await self._fetch("https://www.nseindia.com/market-data/oi-spurts",
                                  "https://www.nseindia.com/api/live-analysis-oi-spurts-underlyings")
        if not isinstance(data, dict) or not isinstance(data.get("data"), list): return None
        df = pd.DataFrame(data["data"])
    ...
    async def fno_live_oi_vs_price(self) -> pd.DataFrame | None:
        ...
        for block in data.get("data",[]):
            for category, contracts in block.items():
                for c in contracts: c["OI_Price_Signal"] = category; rows.append(c)
        if not rows: return None
        df = pd.DataFrame(rows)
    ...
    async def fno_expiry_dates(self, symbol: str = "NIFTY", label_filter: Any = None):
        ...
        for i, date in enumerate(expiry_dates):
            ...
            else: expiry_info.append("Monthly Expiry")
        ...
        df["Contract Zone"] = expiry_dates.apply(zone)
    ...
    @nse_api(ttl=15.0)
    async def fno_live_option_chain(self, symbol: str = "NIFTY",
                                     expiry_date: str | None = None,
                                     oi_mode: str = "full") -> pd.DataFrame:
        sym = self._validate_symbol(symbol)
        if oi_mode == "compact":
            col_names = ["Fetch_Time","Symbol","Expiry_Date","CALLS_OI","CALLS_Chng_in_OI","CALLS_Volume",
                         "CALLS_IV","CALLS_LTP","CALLS_Net_Chng","Strike_Price",
                         "PUTS_OI","PUTS_Chng_in_OI","PUTS_Volume","PUTS_IV","PUTS_LTP","PUTS_Net_Chng","Underlying_Value"]
        else:
            col_names = ["Fetch_Time","Symbol","Expiry_Date","CALLS_OI","CALLS_Chng_in_OI","CALLS_Volume",
                         "CALLS_IV","CALLS_LTP","CALLS_Net_Chng","CALLS_Bid_Qty","CALLS_Bid_Price",
                         "CALLS_Ask_Price","CALLS_Ask_Qty","Strike_Price","PUTS_Bid_Qty","PUTS_Bid_Price",
                         "PUTS_Ask_Price","PUTS_Ask_Qty","PUTS_Net_Chng","PUTS_LTP","PUTS_IV","PUTS_Volume",
                         "PUTS_Chng_in_OI","PUTS_OI","Underlying_Value"]
        ...
        resp_d = await self._request("https://www.nseindia.com/api/NextApi/apiClient/GetQuoteApi",
                                     params={"functionName":"getOptionChainDropdown","symbol":sym},
                                     timeout=10)
        if not resp_d or not resp_d.get("expiryDates"):
            return pd.DataFrame(columns=col_names).astype(dtypes)
        avail = resp_d["expiryDates"]
        if expiry_date:
            try: target = pd.to_datetime(expiry_date, dayfirst=True).strftime("%d-%b-%Y")
            except: target = str(expiry_date).strip()
            if target not in avail: target = avail[0]
        else: target = avail[0]
        payload = await self._request("https://www.nseindia.com/api/NextApi/apiClient/GetQuoteApi",
                                      params={"functionName":"getOptionChainData","symbol":sym,
                                              "params":f"expiryDate={target}"},
                                      timeout=15)
        if not payload: return pd.DataFrame(columns=col_names).astype(dtypes)
        timestamp = payload.get("timestamp", datetime.now().strftime("%d-%b-%Y %H:%M:%S"))
        uv        = payload.get("underlyingValue", 0)
        records   = payload.get("data",[])
        if not records: return pd.DataFrame(columns=col_names).astype(dtypes)
        rows = []
        for item in records:
            ce = item.get("CE",{}); pe = item.get("PE",{}); sp = item.get("strikePrice")
            row = {"Fetch_Time":timestamp,"Symbol":sym,"Expiry_Date":target,"Strike_Price":sp,
                   "CALLS_OI":ce.get("openInterest",0),"CALLS_Chng_in_OI":ce.get("changeinOpenInterest",0),
                   "CALLS_Volume":ce.get("totalTradedVolume",0),"CALLS_IV":ce.get("impliedVolatility",0),
                   "CALLS_LTP":ce.get("lastPrice",0),"CALLS_Net_Chng":ce.get("change",0),
                   "PUTS_OI":pe.get("openInterest",0),"PUTS_Chng_in_OI":pe.get("changeinOpenInterest",0),
                   "PUTS_Volume":pe.get("totalTradedVolume",0),"PUTS_IV":pe.get("impliedVolatility",0),
                   "PUTS_LTP":pe.get("lastPrice",0),"PUTS_Net_Chng":pe.get("change",0),"Underlying_Value":uv}
            if oi_mode == "full":
                row.update({"CALLS_Bid_Qty":ce.get("totalBuyQuantity",0) or ce.get("buyQuantity1",0),
                             "CALLS_Bid_Price":ce.get("buyPrice1",0),"CALLS_Ask_Price":ce.get("sellPrice1",0),
                             ...
                             "PUTS_Bid_Price":pe.get("buyPrice1",0),"PUTS_Ask_Price":pe.get("sellPrice1",0),
    ...
    async def cm_eod_bulk_deal(self, trade_date: str | None = None) -> pd.DataFrame | None:
        raw = await self._get("https://nsearchives.nseindia.com/content/equities/bulk.csv", is_json=False)
        return pd.read_csv(BytesIO(raw)) if raw else None

    async def cm_eod_block_deal(self, trade_date: str | None = None) -> pd.DataFrame | None:
    ...
    @nse_api(ttl=300.0)
    async def cm_hist_security_wise_data(self, *args: Any, symbol=None, from_date=None,
                                          to_date=None, period=None) -> pd.DataFrame | None:
        today = datetime.now(); today_str = today.strftime("%d-%m-%Y")
        for arg in args:
            if not isinstance(arg, str): continue
            if _DATE_RE.match(arg):         from_date = from_date or arg; to_date = to_date or (arg if from_date != arg else None)
        ...
        if not symbol: log.warning("[AsyncNse] symbol required"); return None
        if period:
            from_date = (today-_DELTA_MAP.get(period,timedelta(days=365))).strftime("%d-%m-%Y")
            to_date   = to_date or today_str
        from_date = from_date or (today-timedelta(days=365)).strftime("%d-%m-%Y")
        to_date   = to_date   or today_str
        base_api  = (f"https://www.nseindia.com/api/historicalOR/generateSecurityWiseHistoricalData?"
                     f"from={{}}&to={{}}&symbol={symbol}&type=priceVolumeDeliverable&series=ALL")
        rows = await self._chunk_fetch("https://www.nseindia.com/report-detail/eq_security",
                                        base_api, from_date, to_date)
        if not rows: return None
        df = pd.DataFrame(rows)
        expected = ["CH_SYMBOL","CH_SERIES","mTIMESTAMP","CH_PREVIOUS_CLS_PRICE","CH_OPENING_PRICE",
                    "CH_TRADE_HIGH_PRICE","CH_TRADE_LOW_PRICE","CH_LAST_TRADED_PRICE","CH_CLOSING_PRICE",
                    "VWAP","CH_TOT_TRADED_QTY","CH_TOT_TRADED_VAL","CH_TOTAL_TRADES","COP_DELIV_QTY","COP_DELIV_PERC"]
        df = df[[c for c in expected if c in df.columns]]
        df.rename(columns={"CH_SYMBOL":"Symbol","CH_SERIES":"Series","mTIMESTAMP":"Date",
                            "CH_PREVIOUS_CLS_PRICE":"Prev Close","CH_OPENING_PRICE":"Open Price",
                            "CH_TRADE_HIGH_PRICE":"High Price","CH_TRADE_LOW_PRICE":"Low Price",
                            "CH_LAST_TRADED_PRICE":"Last Price","CH_CLOSING_PRICE":"Close Price",
                            "VWAP":"VWAP","CH_TOT_TRADED_QTY":"Total Traded Quantity",
                            "CH_TOT_TRADED_VAL":"Turnover ₹","CH_TOTAL_TRADES":"No. of Trades",
                            "COP_DELIV_QTY":"Deliverable Qty","COP_DELIV_PERC":"% Dly Qt to Traded Qty"}, inplace=True)
        df.replace({float("inf"):0, float("-inf"):0}, inplace=True); df.fillna(0, inplace=True)
        df["Date"] = pd.to_datetime(df["Date"], format="%d-%b-%Y", errors="coerce")
        df.sort_values("Date", inplace=True); df.drop_duplicates(subset=["Date"], keep="last", inplace=True)
    ...
    async def _csv_deals_fetch(self, api: str,
                                ref_url: str = "https://www.nseindia.com/report-detail/display-bulk-and-block-deals") -> pd.DataFrame | None:
    ...
    @nse_api(ttl=300.0)
    async def cm_hist_bulk_deals(self, *args: Any, from_date=None, to_date=None,
                                   period=None, symbol=None) -> pd.DataFrame | None:
        for arg in args:
            if not isinstance(arg, str): continue
            if _DATE_RE.match(arg):         from_date = from_date or arg
        ...
        api = (f"https://www.nseindia.com/api/historicalOR/bulk-block-short-deals"
               f"?optionType=bulk_deals&from={from_date}&to={to_date}"
               + (f"&symbol={symbol}" if symbol else "") + "&csv=true")
        return await self._csv_deals_fetch(api)

    @nse_api(ttl=300.0)
    async def cm_hist_block_deals(self, *args: Any, from_date=None, to_date=None,
                                    period=None, symbol=None) -> pd.DataFrame | None:
        for arg in args:
            if not isinstance(arg, str): continue
            if _DATE_RE.match(arg):         from_date = from_date or arg
    ...
    @nse_api(ttl=300.0)
    async def cm_hist_short_selling(self, *args: Any, from_date=None, to_date=None,
                                      period=None, symbol=None) -> pd.DataFrame | None:
        for arg in args:
            if not isinstance(arg, str): continue
            if _DATE_RE.match(arg):         from_date = from_date or arg
    ...
    @nse_api(ttl=300.0)
    async def cm_hist_eq_price_band(self, *args: Any, from_date=None, to_date=None,
                                      period=None, symbol=None) -> pd.DataFrame | None:
        for arg in args:
            if not isinstance(arg, str): continue
            if _DATE_RE.match(arg):         from_date = from_date or arg
    ...
    @nse_api(ttl=300.0)
    async def cm_monthly_settlement_report(self, *args: Any, from_year=None, to_year=None, period=None) -> pd.DataFrame | None:
        for arg in args:
        ...
        from_year = from_year or fy_start; to_year = to_year or from_year+1
        all_data: list = []
        for fy in range(from_year, to_year):
            data = await self._fetch("https://www.nseindia.com/report-detail/monthly-settlement-statistics",
    ...
    async def cm_monthly_most_active_equity(self) -> pd.DataFrame | None:
        data = await self._fetch("https://www.nseindia.com/market-data/monthly-most-active-equity",
    ...
    async def fno_eod_fii_stats(self, trade_date: str) -> pd.DataFrame | None:
        ...
        raw = await self._get(f"https://nsearchives.nseindia.com/content/fo/fii_stats_{fmt}.xls", is_json=False)
        if not raw: return None
        buf    = BytesIO(raw)
        ...
        with zipfile.ZipFile(BytesIO(raw)) as zf:
            for name in zf.namelist():
                if name.lower().startswith("ttfut") and name.endswith(".csv"):
                    return list(csv.reader(zf.open(name).read().decode("utf-8", errors="ignore").splitlines()))
        return None
    ...
    async def fno_eod_participant_wise_vol(self, trade_date: str) -> pd.DataFrame | None:
        d   = datetime.strptime(trade_date, "%d-%m-%Y")
        raw = await self._get(
            f"https://nsearchives.nseindia.com/content/nsccl/fao_participant_vol_{d.strftime('%d%m%Y')}.csv",
            is_json=False)
        return pd.read_csv(BytesIO(raw)) if raw else None
    ...
    @nse_api(ttl=300.0)
    async def future_price_volume_data(self, *args: Any, **kwargs: Any) -> pd.DataFrame:
        if len(args) < 2: raise ValueError("At least symbol and instrument must be provided.")
        ...
        expiry_re = re.compile(r"^[A-Z]{3}-\d{2}$", re.IGNORECASE)
        for arg in args[2:]:
            if not isinstance(arg, str): continue
            u = arg.strip().upper()
            if expiry_re.match(u):           expiry = u
            elif _DATE_RE.match(arg):
                if not from_date: from_date = arg
                else:             to_date   = arg
            elif _PERIOD_RE.match(u):        period = u
        expiry   = kwargs.get("expiry",    expiry)
        from_date = kwargs.get("from_date", from_date)
        to_date   = kwargs.get("to_date",   to_date)
        period   = kwargs.get("period",    period)
        if not from_date and not period:
            from_date, to_date = _period_to_dates(period="1M")
        elif period and not from_date:
            from_date, to_date = _period_to_dates(period=period)
        to_date = to_date or datetime.now().strftime("%d-%m-%Y")
        inst_map = {"index futures":"FUTIDX","stock futures":"FUTSTK",
                    "index options":"OPTIDX","stock options":"OPTSTK"}
        inst_code = inst_map.get(instrument, instrument.upper())
        params: dict[str,str] = {"instrumentType":inst_code,"symbol":symbol,
                                  "from":from_date,"to":to_date}
        if expiry: params["expiryDate"] = expiry
        url = ("https://www.nseindia.com/api/historical/fo/derivatives?"
               + "&".join(f"{k}={v}" for k,v in params.items()))
        rows = await self._chunk_fetch("https://www.nseindia.com/market-data/derivatives-market",
                                        url.replace(f"&from={from_date}&to={to_date}",
                                                     "&from={}&to={}"),
                                        from_date, to_date)
        if not rows: return pd.DataFrame()
        df = pd.DataFrame(rows); df.columns = df.columns.str.upper().str.replace(" ","_",regex=False)
        num = ["FH_OPENING_PRICE","FH_TRADE_HIGH_PRICE","FH_TRADE_LOW_PRICE","FH_CLOSING_PRICE",
               "FH_LAST_TRADED_PRICE","FH_PREV_CLS","FH_SETTLE_PRICE","FH_TOT_TRADED_QTY",
               "FH_TOT_TRADED_VAL","FH_OPEN_INT","FH_CHANGE_IN_OI","CALCULATED_PREMIUM_VAL"]
        for c in set(num) & set(df.columns): df[c] = pd.to_numeric(df[c], errors="coerce")
        if "FH_TOT_TRADED_QTY" in df: df = df[df["FH_TOT_TRADED_QTY"] > 0]
        for c, fmt in {"FH_TIMESTAMP":"%d-%b-%Y","FH_EXPIRY_DT":"%d-%b-%Y"}.items():
            if c in df: df[c] = pd.to_datetime(df[c], errors="coerce").dt.strftime(fmt)
        s = [c for c in ("FH_TIMESTAMP","FH_EXPIRY_DT") if c in df]
        return df.sort_values(s).reset_index(drop=True) if s else df

    @nse_api(ttl=300.0)
    async def option_price_volume_data(self, *args: Any, **kwargs: Any) -> pd.DataFrame:
        if len(args) < 2: raise ValueError("At least symbol and instrument must be provided.")
        symbol     = args[0].strip().upper()
        instrument = args[1].strip().lower()
        expiry = strike = opt_type = from_date = to_date = period = None
        _mm = {"JAN":1,"FEB":2,"MAR":3,"APR":4,"MAY":5,"JUN":6,
               "JUL":7,"AUG":8,"SEP":9,"OCT":10,"NOV":11,"DEC":12}
        expiry_re = re.compile(r"^[A-Z]{3}-\d{2}$", re.IGNORECASE)
        for arg in args[2:]:
            ...
            u = arg.strip().upper()
            if expiry_re.match(u):             expiry = u
            elif u in ("CE","PE"):             opt_type = u
            elif re.match(r"^\d+(\.\d+)?$",u): strike = u
            elif _DATE_RE.match(arg):
                if not from_date: from_date = arg
                else:             to_date   = arg
            elif _PERIOD_RE.match(u):          period = u
        ...
        opt_type  = kwargs.get("opt_type",  opt_type)
        from_date = kwargs.get("from_date", from_date)
        to_date   = kwargs.get("to_date",   to_date)
        period    = kwargs.get("period",    period)
        if not from_date and not period:
        ...
        if opt_type: params["optionType"]  = opt_type
        url = ("https://www.nseindia.com/api/historical/fo/derivatives?"
               + "&".join(f"{k}={v}" for k,v in params.items()))
        rows = await self._chunk_fetch("https://www.nseindia.com/market-data/derivatives-market",
                                        url.replace(f"&from={from_date}&to={to_date}",
        ...
        df = pd.DataFrame(rows); df.columns = df.columns.str.upper().str.replace(" ","_",regex=False)
        num = ["FH_OPENING_PRICE","FH_TRADE_HIGH_PRICE","FH_TRADE_LOW_PRICE","FH_CLOSING_PRICE",
               "FH_LAST_TRADED_PRICE","FH_SETTLE_PRICE","FH_TOT_TRADED_QTY","FH_TOT_TRADED_VAL",
    ...
    @nse_api(ttl=15.0)
    async def fno_dmy_biz_growth(self, *args: Any, mode: str = "monthly",
                                   month: int | None = None, year: int | None = None):
        now = datetime.now()
        _mm = {"JAN":1,"FEB":2,"MAR":3,"APR":4,"MAY":5,"JUN":6,"JUL":7,"AUG":8,"SEP":9,"OCT":10,"NOV":11,"DEC":12}
        _rv = {v:k for k,v in _mm.items()}
        ...
        elif mode=="daily":   url = f"https://www.nseindia.com/api/historicalOR/fo/tbg/daily?month={_rv.get(month,str(month)).title()}&year={year}"
        else: raise ValueError("mode must be yearly/monthly/daily")
        data = await self._fetch("https://www.nseindia.com", url)
        if not data: return None
        data_list = [item["data"] for item in (data.get("data") or []) if "data" in item]
        if not data_list: return None
    ...
    @nse_api(ttl=300.0)
    async def fno_monthly_settlement_report(self, from_year: int | None = None,
                                              to_year: int | None = None) -> pd.DataFrame | None:
        now = datetime.now()
        from_year = from_year or now.year; to_year = to_year or now.year
        all_dfs: list = []
        for yr in range(from_year, to_year+1):
            data = await self._fetch("https://www.nseindia.com/option-chain",
                                      f"https://www.nseindia.com/api/monthly-settlement-report?segment=fno&year={yr}")
            if data and isinstance(data.get("data"), list): all_dfs.append(pd.DataFrame(data["data"]))
    ...
    async def sebi_circulars(self, *args: Any, period: str = "1W") -> pd.DataFrame:
        ...
        payload = {"fromDate":from_str,"toDate":to_str,"fromYear":"","toYear":"","deptId":"-1",
                   "sid":"1","ssid":"7","smid":"0","ssidhidden":"7","intmid":"-1","sText":"",
                   "ssText":"Circulars","smText":"","doDirect":"-1","nextValue":"1","nextDel":"1","totalpage":"1"}
        text = await self._request("https://www.sebi.gov.in/sebiweb/ajax/home/getnewslistinfo.jsp",
                                    method="POST", data=payload, extra_headers=hdrs, is_json=False, timeout=15)
        ...
        if table:
            for tr in table.find_all("tr")[1:]:
                ...
                rows.append({"Date":pd.to_datetime(tds[0].text.strip(), errors="coerce"),
    ...
    async def recent_annual_reports(self) -> pd.DataFrame:
        raw = await self._get("https://nsearchives.nseindia.com/content/RSS/Annual_Reports.xml", is_json=False)
        if not raw: return pd.DataFrame()
        ...
        for item in feed.entries:
            ...
            records.append({"symbol":m.group("symbol"),"companyName":item.get("title",""),
                             "fyFrom":int(m.group("fyFrom")),"fyTo":int(m.group("fyTo")),
                             "link":link,"submissionDate":sub,"SME":sme})
        return pd.DataFrame(records)

    async def html_tables(self, url: str, show_tables: bool = False, output: str = "json") -> list | None:
        raw = await self._request(url, ref_url=_NSE_OPTION, is_json=False, timeout=30)
        if not raw: return None
        try:
            ...
            log.warning("[AsyncNse] html_tables failed: %s", exc); return None
    ...
    def rate_limiter_stats(self) -> dict:
        """Convenience wrapper matching sync nse.rate_limiter.stats()."""
    ...
    @nse_api(ttl=30.0)
    async def cm_live_bulk_deals(self, *args: Any, from_date=None, to_date=None, period=None) -> pd.DataFrame | None:
        from_str, to_str = _period_to_dates(*args, from_date=from_date, to_date=to_date, period=period, default_period="1M")
        data = await self._fetch("https://www.nseindia.com/market-data/bulk-deals",
    ...
    @nse_api(ttl=30.0)
    async def cm_live_block_deals(self, *args: Any, from_date=None, to_date=None, period=None) -> pd.DataFrame | None:
        from_str, to_str = _period_to_dates(*args, from_date=from_date, to_date=to_date, period=period, default_period="1M")
        data = await self._fetch("https://www.nseindia.com/market-data/block-deals",
                                  f"https://www.nseindia.com/api/block-deals?from={from_str}&to={to_str}")
        items = (data or {}).get("data") if isinstance(data, dict) else (data if isinstance(data, list) else [])
        return pd.DataFrame(items) if items else None

    @nse_api(ttl=30.0)
    async def fno_eod_fii_dii(self, *args: Any, from_date=None, to_date=None, period=None) -> pd.DataFrame | None:
        from_str, to_str = _period_to_dates(*args, from_date=from_date, to_date=to_date, period=period, default_period="1M")
        data = await self._fetch("https://www.nseindia.com/market-data/fii-dii-trades",
                                  f"https://www.nseindia.com/api/fii-dii?from={from_str}&to={to_str}")
        items = (data or {}).get("data") if isinstance(data, dict) else (data if isinstance(data, list) else [])
        return pd.DataFrame(items) if items else None

    async def cm_live_hist_annual_reports(self, symbol: str, *args: Any,
                                           from_date=None, to_date=None, period=None) -> pd.DataFrame | None:
        sym = self._validate_symbol(symbol)
        from_str, to_str = _period_to_dates(*args, from_date=from_date, to_date=to_date, period=period, default_period="3M")
        data = await self._fetch("https://www.nseindia.com/companies-listing/corporate-filings-annual-reports",
                                  f"https://www.nseindia.com/api/annual-reports?symbol={sym}&from={from_str}&to={to_str}")
        items = (data or {}).get("data",[]) if isinstance(data, dict) else (data if isinstance(data, list) else [])
        return pd.DataFrame(items) if items else None

    async def fetch_many(self, callables: list[Callable[[], Any]], *,
    ...
    async def morning_dashboard(self) -> dict[str, Any]:
        """Fetch 5 core morning metrics in parallel."""
        ...
        return {"market":market,"turnover":turnover,"indices":indices,"vix":vix,"preopen":preopen}
    ...
    async def watchlist_snapshot(self, symbols: list[str]) -> dict[str, dict | None]:
        ...
        return {sym: (r if not isinstance(r, Exception) else None)
...
class AsyncNseQueryBuilder:
    """Fluent builder for AsyncNse queries — mirrors NseQueryBuilder."""
    __slots__ = ("_client","_symbol","_period","_from","_to","_filter","_policy")

    def __init__(self, client: AsyncNse) -> None:
        self._client = client
        self._symbol: str | None = None; self._period: str | None = None
        self._from:   str | None = None; self._to:     str | None = None
        self._filter: str | None = None; self._policy: CachePolicy = CachePolicy.READWRITE

    def symbol(self, s: str) -> "AsyncNseQueryBuilder": self._symbol = s.upper(); return self
    def period(self, p: str) -> "AsyncNseQueryBuilder": self._period = p.upper(); return self
    def date_range(self, f: str, t: str) -> "AsyncNseQueryBuilder": self._from, self._to = f, t; return self
    def filter(self, f: str) -> "AsyncNseQueryBuilder": self._filter = f; return self
    def cache_policy(self, p: CachePolicy) -> "AsyncNseQueryBuilder": self._policy = p; return self
...
# ═══════════════════════════════════════════════════════════════════════════════
# AsyncNseSession context manager factory
# ═══════════════════════════════════════════════════════════════════════════════
...
async def AsyncNseSession(
    *,
    ...
    """
    Async context manager for AsyncNse.
    ...
    ::
        ...
        async with AsyncNseSession(max_per_second=5) as nse:
            df = await nse.index_live_all_indices_data()
    """
    client = AsyncNse(max_per_second=max_per_second, max_per_minute=max_per_minute,
                      min_gap=min_gap, cache_ttl=cache_ttl, cache_size=cache_size)
    await client._open()
    try:
        yield client
    finally:
...
# ═══════════════════════════════════════════════════════════════════════════════
# sync_fetch convenience wrapper
# ═══════════════════════════════════════════════════════════════════════════════
